...
export default function SettingsScreen() {
  ...
  const handleLogout = () => {
    console.log('Logout button pressed. Pending count:', pendingCount);
    if (pendingCount > 0) {
      if (Platform.OS === 'web') {
        const confirm = window.confirm(
          `Pending Collections: You have ${pendingCount} unsynced collections. Logging out will NOT delete them. Continue?`
        );
        if (confirm) logout();
      } else {
        Alert.alert(
...
const styles = StyleSheet.create({
  ...
  logoutButton: {
    ...
    borderColor: '#FECACA',
    cursor: 'pointer', // Fix for web clickability

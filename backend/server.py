...
from fastapi import FastAPI, APIRouter, HTTPException, Depends, Header, Form
...
# --- SECURITY SETUP ---
# load_dotenv is called below after ROOT_DIR is defined, so we defer SECRET_KEY read
# SECRET_KEY is loaded after load_dotenv() is called (see below)
ALGORITHM = "HS256"
...
# SECRET_KEY is now read from .env (generate with: python -c "import secrets; print(secrets.token_hex(32))")
...
async def google_callback(request: Request):
    ...
    try:
        ...
        access_token = create_access_token(
            data={"sub": user["username"], "role": user.get("role", "farmer")}, expires_delta=access_token_expires
        )
...
class BlockchainTransaction(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4())); product_id: str; transaction_type: str; data_hash: str; previous_hash: Optional[str] = None; merkle_root: str; block_index: int; timestamp: datetime = Field(default_factory=lambda: datetime.now(timezone.utc)); blockchain_verified: Optional[bool] = False; fabric_tx_id: Optional[str] = None
...
async def get_user(username: str):
    ...
    if user:
        ...
        try:
            ...
            # Fallback for partially migrated or invalid user docs in DB
            return UserInDB(
...
async def get_current_user(token: str = Depends(oauth2_scheme)):
    ...
    # --- DEMO MODE BYPASS ---
    if os.environ.get('DEMO_MODE') == 'true' and (token.endswith('.fake_signature') or token == 'demo_token'):
        try:
            # Manually decode the payload if it's a fake JWT
            if '.' in token:
                parts = token.split('.')
                if len(parts) >= 2:
                    payload_b64 = parts[1]
                    ...
                    role = payload.get("role", "aggregator")
                    return User(username=username, organization="Demo Org", role=role)
            return User(username="demo_user", organization="Demo Org", role="aggregator")
        except Exception:
    ...
    if user is None:
        ...
        if collector:
            return User(username=collector["collector_id"], organization=collector.get("organization"), role="farmer")
...
def generate_qr_code(product_id: str) -> tuple[str, str]:
    frontend_url = os.environ.get('FRONTEND_URL', 'http://10.60.47.252:3000'); qr_data = f"{frontend_url}/trace/{product_id}"
...
async def create_blockchain_transaction(product_id: str, transaction_type: str, data: dict):
    last_tx = await db.blockchain_transactions.find_one({"product_id": product_id}, sort=[("block_index", -1), ("block_number", -1)])
    previous_hash = last_tx.get("data_hash") if last_tx else "0"
    if last_tx:
        last_index = last_tx.get("block_index") if "block_index" in last_tx else last_tx.get("block_number", 0)
    ...
    try:
        ...
        if is_connected:
            fabric_result = None
            if transaction_type in ("collection", "hardware_collection"):
                fabric_result = await fabric_service.record_collection(data)
            elif transaction_type == "processing":
                fabric_result = await fabric_service.record_processing(data)
            elif transaction_type == "quality":
                fabric_result = await fabric_service.record_quality_test(data)
            elif transaction_type == "product":
                fabric_result = await fabric_service.record_product(data)
                
            if fabric_result and fabric_result.get("success"):
                blockchain_verified = True
                fabric_tx_id = fabric_result.get("collection_id") or fabric_result.get("processing_id") or fabric_result.get("test_id") or fabric_result.get("product_id") or "FABRIC-TX"
                print(f"⛓️ [Blockchain] Successfully committed '{transaction_type}' transaction to Hyperledger Fabric!")
    except Exception as e:
        print(f"⚠️ [Blockchain Warning] Hyperledger Fabric commit failed (relying on simulated fallback): {str(e)}")
...
async def register_user(
    ...
    user_in_db = UserInDB(username=username, organization="Default Org", role=role, hashed_password=hashed_password)
...
async def login_for_access_token(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    access_token_expires = timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)
    role_to_use = form_data.client_id if form_data.client_id else getattr(user, 'role', 'farmer')
    access_token = create_access_token(data={"sub": user.username, "role": role_to_use}, expires_delta=access_token_expires)
    return {"access_token": access_token, "token_type": "bearer"}
...
async def create_collection_event(event: CollectionEvent, current_user: User = Depends(get_current_user)):
    # FRAUD DETECTION: GPS Cross-Reference with Farm Profile
    collector = await db.collectors.find_one({"collector_id": event.collector_id})
    if collector and collector.get("farm_location"):
        farm_lat = collector["farm_location"]["latitude"]
        farm_lon = collector["farm_location"]["longitude"]
        
        # Calculate distance between collection point and registered farm
        dist_km = calculate_haversine_distance(event.latitude, event.longitude, farm_lat, farm_lon)
        
        # If distance > 0.5km (500 meters), reject as fraud
        if dist_km > 0.5:
            raise HTTPException(
                status_code=400, 
                detail=f"FRAUD DETECTED: Collection location mismatch. You are {dist_km}km away from your registered farm."
            )
...
async def get_dashboard_analytics():
    ...
    recent_tests_raw = await db.quality_tests.find().sort("timestamp", -1).limit(10).to_list(10)
    def clean_mongo_doc(doc):
        if doc and '_id' in doc: del doc['_id']
        return doc
    return {
        ...
        "recent_quality_tests": [clean_mongo_doc(doc) for doc in recent_tests_raw]
    }

@api_router.get("/quality-tests")
async def get_quality_tests(limit: int = 20, product_id: Optional[str] = None):
    """Get recent quality tests — used by Lab Dashboard."""
    query = {}
    if product_id:
        query["product_id"] = product_id
    tests_raw = await db.quality_tests.find(query).sort("timestamp", -1).limit(limit).to_list(limit)
    def clean_mongo_doc(doc):
        if doc and '_id' in doc: del doc['_id']
        return doc
    return {"tests": [clean_mongo_doc(doc) for doc in tests_raw], "total": len(tests_raw)}
...
async def get_recent_transactions(limit: int = 15):
    """
    Get all recent transactions recorded on the blockchain.
    Used by the live Blockchain Explorer frontend component.
    """
    try:
        txs = await db.blockchain_transactions.find().sort("timestamp", -1).limit(limit).to_list(limit)
        for tx in txs:
            if '_id' in tx:
                del tx['_id']
        return txs
...
async def trace_product_on_blockchain(product_id: str):
    ...
    try:
        ...
        # If blockchain returned empty product/collections, throw exception to trigger full MongoDB fallback
        if not result.get("product") and not result.get("collections") and not result.get("trace_data", {}).get("collections"):
            raise Exception("No data on blockchain, falling back to full MongoDB trace")

        # Even if blockchain has collections, it might be missing processing/quality tests. Enrich them from MongoDB.
        batch_id_to_search = product_id
        if result.get("product"):
            batch_id_to_search = result["product"].get("batch_id", product_id)

        if not result.get("quality_tests") and not result.get("trace_data", {}).get("quality_tests"):
            q_tests = await db.quality_tests.find({"$or": [{"product_id": batch_id_to_search}, {"product_id": product_id}]}).to_list(1000)
            if q_tests:
                for t in q_tests: t.pop("_id", None)
                result["quality_tests"] = q_tests
                result["total_events"] = result.get("total_events", 0) + len(q_tests)

        if not result.get("processing_steps") and not result.get("processing") and not result.get("trace_data", {}).get("processing_steps"):
            p_steps = await db.processing_steps.find({"$or": [{"product_id": batch_id_to_search}, {"product_id": product_id}]}).to_list(1000)
            if p_steps:
                for p in p_steps: p.pop("_id", None)
                result["processing_steps"] = p_steps
                result["total_events"] = result.get("total_events", 0) + len(p_steps)
...
async def startup_event():
    ...
    try:
        is_connected = await fabric_service.check_connection()
        if is_connected:
            print("[OK] Hyperledger Fabric blockchain: CONNECTED")
            print("     Channel: herblock | Chaincode: herblock")
        else:
            print("[--] Hyperledger Fabric: Not connected (MongoDB fallback active)")
    except Exception as e:
    ...
    print("  [OK] GPS Geo-Fence Validation (Patent)")
    print("  [OK] Immutable Quality Records (SHA-256)")
    print("  [OK] Multi-Org Consensus (Org1 + Org2)")
    print("  [OK] Supply Chain Traceability")
    print("")
...
async def get_collector_profile(collector_id: str):
    ...
    return {
        ...
        "organization": collector.get("organization"),
        "registered_species": collector.get("registered_species", []),
        "farm_photos": collector.get("farm_photos", []),
        "total_collections": total
    }

class FarmPhotosUpload(BaseModel):
    photos: List[str]
    location: Optional[Dict[str, float]] = None

@collector_router.post("/{collector_id}/farm-photos")
async def upload_farm_photos(collector_id: str, payload: FarmPhotosUpload):
    """Upload reference photos of the farm for Visual Verification"""
    collector = await db.collectors.find_one({"collector_id": collector_id})
    if not collector:
        raise HTTPException(status_code=404, detail="Collector not found")
    ...
    await db.collectors.update_one(
        ...
        {"$set": {
            "farm_photos": payload.photos,
            "farm_location": payload.location
        }}
    )
    
    return {"success": True, "message": "Farm photos and location updated successfully", "count": len(payload.photos)}

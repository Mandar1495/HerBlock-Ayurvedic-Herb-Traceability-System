...
export default function HomeScreen({ navigation }) {
  ...
  // Check network status & check farm setup
  useEffect(() => {
    ...
    // Redirect to FarmSetup if they don't have farm photos
    const hasPhotos = collector?.farm_photos && collector.farm_photos.length >= 2;
    if (collector && !hasPhotos) {
      const timer = setTimeout(() => {
        // Double check condition before navigating
        if (!hasPhotos) {
  ...
  return (
    ...
    >
      ...
      <TouchableOpacity 
        ...
        onPress={() => {
          if (!collector?.farm_photos || collector.farm_photos.length < 3) {
            navigation.navigate('FarmSetup');
          } else {
      ...
      >
        ...
        <View style={styles.actionTextContainer}>
          <Text style={styles.actionTitle}>Farm Visual Setup</Text>
          <Text style={styles.actionSubtitle}>
            {collector?.farm_photos?.length >= 3 ? '✅ Farm Verified' : '⚠️ Action Required'}
          </Text>
        </View>
        <Text style={styles.actionArrow}>→</Text>
      </TouchableOpacity>
